...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        """
        newnd = (id, name)
        if newnd in self.nodes:
            pass
        else:
            self.nodes.append(newnd)
        return self.nodes
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        """
        newed = (source, target)
        inverted_newed = (target, source)
        if newed in self.edges or inverted_newed in self.edges:
            pass
        else:
            self.edges.append(newed)
        return self.edges
    ...
    def node_with_edges(self) -> dict:
        nwe_dict = dict()
        # collect all source nodes and start with value 1
        for i in self.edges:
            if i[0] not in nwe_dict.keys():
                nwe_dict[i[0]] = 1
            else:
            # find the max
                nwe_dict[i[0]] = nwe_dict.get[i[0]] + 1
        # collect target nodes
            if i[1] not in nwe_dict.keys():
                nwe_dict[i[1]] = 1
            else:
                nwe_dict[i[1]] = nwe_dict.get[i[1]] + 1
        return nwe_dict
    ...
    def max_degree_nodes(self) -> dict:
        ...
        or {'a': 22, 'b': 22}
        """
        allnodes = self.node_with_edges()
        alldegree = allnodes.values()
        maxdegree = max(alldegree)
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='02a3cb7f63e32ff489d1178688440a08')

    laurance_credits = tmdb_api_utils.get_movie_credits_for_person(person_id='2975', vote_avg_threshold=8.0)

    for movie in laurance_credits:
    ...
    while loop < 2:
        ...
        for x in tmp_node:
            uniquenode.append(x)
        for c in uniquenode:
            credits = tmdb_api_utils.get_movie_credits_for_person(person_id=str(c[0]), vote_avg_threshold=8.0)
            for movie in credits:
                coactor = tmdb_api_utils.get_movie_cast(movie_id = str(movie['id']), limit = 3)                
                for actor in coactor:
                    if str(actor['id']) != str(c[0]):
                        graph.add_node(id = str(actor['id']), name = actor['name'].replace(',',''))
                        graph.add_edge(source = str(c[0]), target = str(actor['id'])) 
        loop = loop + 1
