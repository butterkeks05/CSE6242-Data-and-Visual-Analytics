...
class HW2_sql():
    ...
    def part_aii_2(self,connection, path):
        ############### CREATE IMPORT CODE BELOW ############################
        curs = connection.cursor() ## gives us the ability to have multiple separate working environments through the same connection to the database 
        with open(path, 'r') as moviecast_data:
            cm = csv.reader(moviecast_data)
            mcd_list = list(cm)
            for i in mcd_list:
    ...
    def part_c(self,connection):
        ...
        part_c_sql = """ SELECT printf( "%.2f",
                        ((SELECT (count(id) * 1.0) FROM movies WHERE score >= 7 AND score <= 20)/
    ...
    def part_h(self,connection,path):
        ...
        with open(path, 'r') as movie_overview_data:
            overview = csv.reader(movie_overview_data)
            ov_list = list(overview)
            for i in ov_list:
    ...
    def part_hi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hi_sql = """   SELECT count(*) as count
                            FROM movie_overview
    ...
    def part_hii(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hii_sql = """  SELECT count(*) as count
                            FROM movie_overview
